...
## 0f. A second, separate problem: multiple GHL app versions with inconsistent scopes (2026-08-24)

Same day, a different failure surfaced: visiting `/api/oauth-connect` directly returns **"Invalid scope(s): conversations.readonly, conversations/message.readonly, locations/tags.readonly"** — three scopes that were part of the *original* working scope list, well before any of the 2026-08-21 changes. This is unrelated to the token-expiry issue above; it happens at the authorize-redirect step itself, before any token exists to expire.

Root cause (as best understood — not fully confirmed): the user has published **multiple versions of the GHL marketplace app** (v1 original, v2 with `customFields` and other scopes added by cloning), and whichever version GHL treats as "live" for new authorizations doesn't have a consistent scope set — it's missing scopes that used to work. The user has been working around this by fully uninstalling and reinstalling the app (~30 times), which apparently resolves it temporarily each time, but the underlying inconsistency isn't fixed by that — it just happens to land on a working version sometimes.

**What was fixed in code:** `locations/tags.readonly` was confirmed via grep to be completely unused anywhere in this codebase, so it's been dropped from the scope request — one less thing that can be "invalid."

**What's NOT fixed, and can't be fixed in code:** `conversations.readonly` / `conversations/message.readonly` are still requested because they're genuinely used (Health dashboard's conversation count + last-message preview in `AccountModal.jsx`) — dropping them would silently break a working feature just to unblock this one. This needs a **GHL-side fix**: whichever app version is actually "live" for new installs needs both of these scopes enabled under Settings → Scopes, consistently with `customFields`. Until that's done, `/api/oauth-connect` will keep hitting this exact error.

**Recommended real fix, not yet done:** stop maintaining multiple app versions/clones. Pick ONE app as canonical, verify it has the full scope set enabled (`contacts.readonly`, `conversations.readonly`, `conversations/message.readonly`, `locations.readonly`, `locations/customFields.readonly`, `opportunities.readonly`, `users.readonly`), and retire/stop using any other version. The repeated uninstall/reinstall cycle is a symptom of this inconsistency, not a real fix — worth raising with whoever manages the GHL Marketplace app once there's time to sort it out properly, rather than continuing the workaround indefinitely.
