  ...
  if (status === 'in_progress') return (
    <div className="flex items-center gap-1.5">
      ...
      <button onClick={() => setStatus(callId, null)}
        ...
        style={{ color: '#6B7280', borderColor: '#E5E7E5' }} title="Reset to Action Required">↩</button>
    </div>
  )
  return (
    <div className="flex items-center gap-1.5">
      ...
      <div className="px-6 py-4 border-b border-brand-border flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-brand-heading font-semibold text-sm flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-brand-red animate-pulse-dot inline-block"
              style={{ boxShadow: '0 0 5px rgba(239,68,68,0.5)' }} />
            Frustrated Calls — Action Required
          </h2>
          <p className="text-brand-muted text-[11px] mt-0.5">
            {calls.length === 0 ? 'No frustrated calls in this period'
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {resolved > 0 && (
              <span className="text-[11px] font-bold px-2.5 py-1 rounded-full border"
                style={{ color: G, background: `${G}10`, borderColor: `${G}25` }}>✓ {resolved} resolved</span>
            )}
            {inProgress > 0 && (
              ...
              return (
                <>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-brand-muted text-[12px]">
                  ...
                  {/* ── Summary sub-row (always visible for frustrated calls) ── */}
                  {call.summary && (
                    <tr
                      key={`summary-${call.id}`}
                    ...
                    >
                      <td colSpan={7} className="pl-10 pr-6 py-2.5">
